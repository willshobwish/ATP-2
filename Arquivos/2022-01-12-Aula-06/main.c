...
int main(void)
{
    ...
    fgets(nomeArquivoAscii,TAMANHONOME,stdin);
    tamanhoEntradaNome = strlen(nomeArquivoAscii) - 1;
    nomeArquivoAscii[tamanhoEntradaNome] = '\0';

    if((fileraw = fopen(nomeArquivoRaw,"r")) == NULL){
        //Abertura do arquivo em RAW
        printf("Erro na abertura do arquivo PGM RAW '%s'",nomeArquivoRaw);
        //Caso nao consiga abrir ele retornara um erro 
        exit(1);
        //Saida do programa
    }

    if((fileascii = fopen(nomeArquivoAscii,"w")) == NULL){
        //Abertura do arquivo em ASCII
        printf("Erro na abertura do arquivo PGM ASCII '%s'",nomeArquivoAscii);
        //Caso nao consiga abrir ele retornara um erro 
        exit(1);
        //Saida do programa
    }
    ...
    fscanf(fileraw,"%100[^\n]",comentario);
    //Leitura de comentario, no maximo 100 caracteres, ate encontrar quebra de linha
    fscanf(fileraw,"%i %i\n",&largura,&altura);
    //Leitura da largura e altura
    fscanf(fileraw,"%i\n",&brilhoMaximo);
    //Leitura do valor de brilho maximo de um pixel
    ...
    //Escrita da largura e altura
    fprintf(fileascii,"%i\n",brilhoMaximo);
    //Escrita do valor de brilho maximo de um pixel

    //Conversao do pixel para ASCII
    for(alturaCont = 0;alturaCont<altura;alturaCont++){
        //for para percorrer a linha
        for(larguraCont = 0;larguraCont<largura;larguraCont++){
            //for para percorrer a coluna
            fscanf(fileraw,"%c",&pixel);
            //Leitura de um pixel do arquivo PGM
            fprintf(fileascii,"%i\n",pixel);
            //Escrita em numero do pixel em arquivo ASCII
        }
    }
