...
int main(void) {
    ...
    if ((fascii = fopen(nome_ascii, "rb")) == NULL) {  // abertura de arquivo texto para escrita
        printf("Erro na abertura do arquivo PGM ascii %s", nome_ascii);
        exit(1);
    }

    if ((fraw = fopen(nome_raw, "w")) == NULL) {  // abertura de arquivo texto para leitura
    ...
    fprintf(fraw, "# Arquivo convertido em ATP II\n");  // insercao de comentario, opcional
    ...
    for (alt = 0; alt < altura; alt++) {          // percorrendo as linhas
        for (larg = 0; larg < largura; larg++) {  // percorrendo as colunas
            fscanf(fascii, "%hu\n", &pixel);      // lendo um byte (caracter), 1 pixel em PGM raw
            fprintf(fraw, "%c", pixel);           // escrita com conversao para texto "legivel", 1 pixel por linha
        }
    }
